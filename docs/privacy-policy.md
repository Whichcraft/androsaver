...
- Never transmitted to the app developer or any unrelated third party; they are sent only to the configured provider when needed for that provider's API.
...
Photos are fetched from your configured sources, displayed on screen, and cached locally on the device (up to 200 images / 150 MiB) as an offline fallback. The Static Image browser may copy one selected image into app-private storage so it remains available after a remote URL expires. Bundled default images and device-local MediaStore photos never leave the device. Public cloud and update traffic use HTTPS. Self-hosted sources default to HTTPS with certificate validation; Synology also supports HTTP-only NAS installations and self-signed/expired certificates on the configured endpoint. The app developer never receives copies of your photos. Photos are never uploaded, analysed, or shared.
...
Each time the Settings screen opens, standard builds download a small version manifest from the selected GitHub Release channel. Standard builds may also download an APK when you explicitly accept an update. The Play Store variant receives updates through Google Play and does not use the GitHub updater. No credentials, photos, or device-identifying application data are deliberately included in those requests.
...
| GitHub Releases | Check for app updates in standard builds | Standard builds, on Settings open |

When background prefetch is enabled by the app lifecycle, the configured image
providers may also be queried after boot or on the WorkManager schedule to
warm the offline cache. This uses the same configured credentials and source
boundaries as the foreground slideshow; it does not upload images.

---

## 4. Local Storage

All configuration data (credentials, settings, image cache, and any copied
static image) is stored locally on the device in app-private storage. It is
not backed up to Google's cloud backup by default. It is deleted when the app
is uninstalled. You can disable a source or clear its credentials from the
Settings screens; Android's app-storage controls can clear the remaining
cache and copied image.
