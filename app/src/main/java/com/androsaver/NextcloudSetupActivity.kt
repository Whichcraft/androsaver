...
class NextcloudSetupActivity : AppCompatActivity() {
    ...
    private fun testConnection() {
        if (!validateRequired()) return
        val port = validatedPort() ?: return
        val config = NextcloudSource.ConnectionConfig(
            host = binding.hostEdit.text.toString().trim(),
            port = port,
            username = binding.usernameEdit.text.toString(),
            password = binding.passwordEdit.text.toString(),
            folder = binding.folderEdit.text.toString().trim().ifEmpty { "/Photos" },
        ...
        lifecycleScope.launch {
            try {
                val ok = withTimeout(60_000L) { NextcloudSource(this@NextcloudSetupActivity).probeConnection(config) }
