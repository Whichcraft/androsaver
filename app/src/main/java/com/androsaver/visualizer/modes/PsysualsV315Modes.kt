...
abstract class ScalarFieldMode : BaseMode() {
    ...
    protected fun clearTrail(draw: GLDraw, alpha: Float) = draw.fadeBlack(alpha / 255f)
...
class PhasonMode : ScalarFieldMode() {
    ...
    override fun draw(draw: GLDraw, audio: AudioData, tick: Int) {
        ...
        hue = (hue + 0.002f) % 1f
        for (y in 0 until rows) for (x in 0 until cols) {
            val nx = x / (cols - 1f) - 0.5f
            val ny = y / (rows - 1f) - 0.5f
...
class CymaticaMode : ScalarFieldMode() {
    ...
    override fun draw(draw: GLDraw, audio: AudioData, tick: Int) {
        ...
        for (i in 0 until 80) {
            ...
            draw.circle(x, y, 1.2f + audio.beat, 0.75f, 0.9f, 1f, 0.7f, true, 5)
        }
        draw.setNormalBlend()
    }
}
...
class MandelboxMode : ScalarFieldMode() {
    ...
    override fun draw(draw: GLDraw, audio: AudioData, tick: Int) {
        ...
        hue = (hue + 0.001f) % 1f
        for (y in 0 until rows) for (x in 0 until cols) {
            var zx = (x / (cols - 1f) - 0.5f) * 2.8f
            var zy = (y / (rows - 1f) - 0.5f) * 2.2f
            var escaped = 0
            for (i in 0 until 14) {
